...
- **US-9.2** Statistiques par faction jouée (parties, victoires) — **M** — dép. US-9.1. — ✅ **Livrée (2026-08-22)**, précédée d'un prérequis non prévu
  ...
  > **Prérequis livré d'abord** : `src/lib/factions.ts` (28 factions AoS groupées par Grande Alliance, `matchFaction()` qui ramène une saisie libre vers l'entrée officielle et renvoie `null` plutôt que de deviner) et `src/components/faction-picker.tsx`, branché sur la soumission de liste **et** sur le profil — deux saisies libres pour la même notion, c'était deux vocabulaires. **La liste des factions reste à faire valider par le porteur, expert AoS.**
  ...
  > **Proposition de l'agent `ux-ui`, à arbitrer** — US-9.3 : déclarer sa faction directement sur la fiche d'un tournoi, sans passer par une liste d'armée. Sans elle, la section restera anecdotique : le gros du trou de couverture, ce sont les tournois qui ne demandent pas de liste.
